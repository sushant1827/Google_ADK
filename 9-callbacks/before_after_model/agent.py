...
def before_model_callback(
    ...
    print(f"===== MODEL REQUEST FROM {agent_name} =====")
    if last_user_message:
        print(f"User Message: {last_user_message[:100]}")
        # Store the last user message in the session state
        state["last_user_message"] = last_user_message

    else:
        print("User Message: None")
        
    print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}")


    # Check if the request contains inappropriate content
    if last_user_message and "sucks" in last_user_message.lower():
        ...
        print("[BEFORE MODEL CALLBACK] Request blocked due to inappropriate content.")
    ...
    # Return None to continue with the model call
    return None
...
def after_model_callback(
    ...
    # Log completion
    print("[AFTER MODEL CALLBACK] Processing response...")

    # Skip processing if the is empty or has no text content
    ...
    for parts in llm_response.content.parts:
        if hasattr(parts, "text") and parts.text:
    ...
    # If modified, update the response
    if modified:
        print("[AFTER MODEL CALLBACK] Replacing negative words with positive alternatives...")
